...
import { getPrComments, getCiFailures, resolveThread, getGitStatusWithPr } from "../services/git.js";
...
export function PrComments() {
  ...
  const selectedThread = !isCiSelected ? threads[selected - ciFailures.length] : null;
  ...
  if (ciFailures.length > 0) headerParts.push(`${ciFailures.length} CI failure${ciFailures.length !== 1 ? "s" : ""}`);
  if (threads.length > 0) headerParts.push(`${threads.length} unresolved thread${threads.length !== 1 ? "s" : ""}`);
  const headerSummary = headerParts.length > 0 ? headerParts.join(", ") : "No issues found";
        ...
        <Box flexDirection="column" width="40%">
          {totalItems === 0 && !loading ? (
            ...
            <>
              {/* CI Failures */}
              {loadingCi && (
                <Text dimColor>   Loading CI failures...</Text>
              )}
              {ciFailures.map((f, i) => {
            ...
            {selectedCi && (
              <>
                ...
                {selectedCi.log ? (
                  <>
                    <Box marginTop={1} />
                    <Text wrap="wrap">{selectedCi.log}</Text>
                  </>
                ) : (
                  <>
                    <Box marginTop={1} />
            ...
            {selectedThread && (
              <>
                <Text bold color="cyan">
                  {selectedThread.path}
                  <Text dimColor>
                    :{selectedThread.startLine && selectedThread.startLine !== selectedThread.line
